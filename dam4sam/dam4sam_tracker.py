...
from dam4sam.sam2.build_sam import build_sam2_video_predictor
...
class DAM4SAMTracker():
    ...
    def _prepare_image(self, img_pil):
        ...
        img = self.to_tensor_transform(img_pil).to(self.inference_state["device"])
        img = F.resize(img, (self.input_image_size, self.input_image_size))
        img = (img - self.img_mean) / self.img_std        
        return img

    def _prepare_all_images(self, img_pil_list):
        imgs = [self.to_tensor_transform(img_pil).to(self.inference_state["device"]) for img_pil in img_pil_list]
        batch = torch.stack(imgs, dim=0)
        batch = torch.nn.functional.interpolate(batch, size=(self.input_image_size, self.input_image_size))
        batch = (batch - self.img_mean[None]) / self.img_std[None]   
        return batch.split(1, dim=0)
    ...
    @torch.inference_mode()
    def initialize(self, image, init_mask, subjects_info=None, batch_size=1, use_fp16=True):
        """
        ...
        self.object_sizes = [] # List to store object sizes (number of pixels) - kept for backward compatibility
        self.object_sizes_per_obj = {} # Dictionary to store object sizes per object ID
        self.last_added = -1 # Frame index of the last added frame into DRM memory - kept for backward compatibility
        self.last_added_per_obj = {} # Dictionary to store last added frame per object ID
        
        self.img_width = image[0].width # Original image width
        self.img_height = image[0].height # Original image height
        self.inference_state = self.init_state_tw()
        # self.inference_state["images"] = image
        video_width, video_height = image[0].size 
        ...
        for frame_idx in unique_frames:
            prepared_img = self._prepare_image(image[frame_idx])
            self.inference_state["images"][frame_idx] = prepared_img
        self.inference_state["num_frames"] = 1
        self.predictor.reset_state(self.inference_state)

        # warm up the model
        self.predictor._get_image_feature(self.inference_state, frame_idx=0, batch_size=1) #batch_size)

        if init_mask is None:
            if subjects_info is None:
        ...
        with torch.autocast(device_type='cuda', dtype=torch.float16, enabled=use_fp16):
            for obj_id in subjects_info:
                obj_info = subjects_info[obj_id]
                for frame_idx in obj_info['frame_info']:
                    frame_obj_info = obj_info['frame_info'][frame_idx]
                    bbox_ = frame_obj_info.get('bbox', None)
                    labels_ = frame_obj_info.get('labels', None)
                    points_ = frame_obj_info.get('points', None)

                    self.predictor._get_image_feature(self.inference_state, frame_idx=frame_idx, batch_size=1) #batch_size)
                    init_mask = self.estimate_mask_from_box(bbox_, labels_, points_, frame_idx)
                    ...
                    if frame_idx==0:
                        m = (out_mask_logits[obj_id, 0] > 0).float().cpu().numpy().astype(np.uint8)
                        out_dict[obj_id] = m

        # for frame_idx in unique_frames:
        #     for i, obj in enumerate(subjects_info):
        #         bbox_ = obj.get('bbox', None)
        #         obj_id = obj.get('id', None)
        #         obj_frame_idx = obj.get('frame', None)

        #         self.predictor._get_image_feature(self.inference_state, frame_idx=frame_idx, batch_size=1) #batch_size)
        #         init_mask = self.estimate_mask_from_box(bbox_, frame_idx)

        #         if obj_frame_idx != frame_idx:
        ...
        #         if obj_frame_idx == frame_idx and obj_frame_idx==0:
        #             m = (out_mask_logits[obj_id, 0] > 0).float().cpu().numpy().astype(np.uint8)
        #             out_dict[obj_id] = m

        self.inference_state["consolidated_frame_inds"]['reference_frames_outputs'] = set(unique_frames)

        # remove the image from the inference state
        # self.inference_state["images"] = {}
    ...
    @torch.inference_mode()
    def track(self, image, use_fp16, init=False):
        """
        ...
        with torch.autocast(device_type='cuda', dtype=torch.float16, enabled=use_fp16):
            for out in self.predictor.propagate_in_video(self.inference_state, start_frame_idx=self.frame_index, max_frame_num_to_track=0, return_all_masks=True):
                if len(out) == 3:
                    ...
                    out_frame_idx, obj_ids, out_mask_logits = out
                    ...
                    out_frame_idx, obj_ids, out_mask_logits, alternative_masks_ious = out
                    m_all = []
                    for obj_id in obj_ids:
                        m = (out_mask_logits[obj_id][0] > 0.0).float().cpu().numpy().astype(np.uint8)
                        m_all.append(m)

                        alternative_masks, out_all_ious = alternative_masks_ious # Extract all predicted masks (chosen and alternatives) and IoUs
                        alternative_masks = alternative_masks[obj_id]
                        out_all_ious = out_all_ious[obj_id]
                        ...
                        # Initialize per-object tracking if needed
                        if obj_id not in self.object_sizes_per_obj:
                            self.object_sizes_per_obj[obj_id] = []
                        if obj_id not in self.last_added_per_obj:
                            self.last_added_per_obj[obj_id] = -1
                        
                        # Keep backward compatibility with global tracking
                        self.object_sizes.append(n_pixels)
                        # Add to per-object tracking
                        self.object_sizes_per_obj[obj_id].append(n_pixels)
                        
                        # Calculate object-specific size ratio
                        obj_sizes = self.object_sizes_per_obj[obj_id]
                        if len(obj_sizes) > 1 and n_pixels >= 1:
                            obj_sizes_ratio = n_pixels / np.median([
                                size for size in obj_sizes[-300:] if size >= 1
                            ][-10:])
                        else:
                            obj_sizes_ratio = -1
                        ...
                        if m_iou > 0.8 and obj_sizes_ratio >= 0.8 and obj_sizes_ratio <= 1.2 and n_pixels >= 1 and (self.frame_index - self.last_added_per_obj[obj_id] > 5 or self.last_added_per_obj[obj_id] == -1):
                            ...
                            if len(alternative_masks) > 0:
                                ...
                                # TODO: visualize the alternative masks
                                ...
                                if np.min(np.array(ious)) <= 0.7:
                                    self.last_added = self.frame_index # Update the last added frame index (backward compatibility)
                                    self.last_added_per_obj[obj_id] = self.frame_index # Update the last added frame index for this object
            ...
            # out_dict = {'pred_mask': m_all}
            out_dict = dict(zip(obj_ids, m_all))

            self.inference_state["images"].pop(self.frame_index)
            return out_dict

    def estimate_mask_from_box(self, bbox, labels, points, frame_idx):
        (
            _,
            _,
        ...
        if points is None:
            points = torch.zeros(0, 2, dtype=torch.float32, device=current_vision_feats[0].device)
        elif not isinstance(points, torch.Tensor):
            points = torch.tensor(points, dtype=torch.float32, device=current_vision_feats[0].device)

        if labels is None:
            labels = torch.zeros(0, dtype=torch.int32, device=current_vision_feats[0].device)
        elif not isinstance(labels, torch.Tensor):
            labels = torch.tensor(labels, dtype=torch.int32, device=current_vision_feats[0].device)
        ...
        from dam4sam.sam2.utils.transforms import SAM2Transforms
        ...
        points_coords = _transforms.transform_coords(
            points, normalize=True, orig_hw=(self.img_height, self.img_width)
        )
        ...
        if bbox is not None:
            ...
            points_coords = torch.cat([box_coords, points_coords], dim=1)
            labels = torch.cat([box_labels, labels], dim=1)

        concat_points = (points_coords, labels)
