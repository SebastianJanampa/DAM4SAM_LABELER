...
from dam4sam.sam2.modeling.sam2_base import NO_OBJ_SCORE, SAM2Base
from dam4sam.sam2.utils.misc import concat_points, fill_holes_in_mask_scores, load_video_frames
...
class SAM2VideoPredictor(SAM2Base):
    ...
    # TODO perhaps add a batch / obj idx to this as optional argument?
    def _consolidate_temp_output_across_obj(
        ...
        for obj_idx in range(batch_size):
            ...
            unbatched_out = unbatch_dict(out)
            if len(unbatched_out) > 1:
                out = unbatched_out[obj_idx]
            else:
                out = unbatched_out[0]
            ...
            consolidated_out["n_pixels_pos"][obj_idx : obj_idx + 1] = out["n_pixels_pos"] #torch.tensor(out["n_pixels_pos"], dtype=consolidated_out["n_pixels_pos"].dtype)
            consolidated_out["iou"][obj_idx : obj_idx + 1] = out["iou"] #torch.tensor(out["iou"])
    ...
    def propagate_in_video(
        ...
        for frame_idx in processing_order:
            ...
            if frame_idx in consolidated_frame_inds["cond_frame_outputs"] and \
                frame_idx not in consolidated_frame_inds["reference_frames_outputs"]:
            ...
            if return_all_masks and "all_pred_masks" in output_dict[storage_key][frame_idx]:
                ...
                all_ious = output_dict[storage_key][frame_idx]["all_pred_masks"][1].reshape(-1, 3)
                all_masks = []
                
                # before = np.sum(video_res_masks.cpu().numpy())
                if len(obj_ids) == 1:
                    all_pred_masks = [all_pred_masks]
                for object_masks in all_pred_masks:
                    all_masks_per_object = []
                    for pred_mask_idx in range(len(object_masks)):
                        _, video_res_masks = self._get_orig_video_res_output(
                            inference_state, object_masks[pred_mask_idx].unsqueeze(0).unsqueeze(0)
                        )
                        all_masks_per_object.append(video_res_masks)
                    all_masks.append(all_masks_per_object)
    ...
    def _run_single_frame_inference(
        ...
        # Compute n_pixels_pos for each object (not just the first one)
        # TODO
        batch_size = pred_masks_gpu.shape[0] 
        n_pixels_pos_per_obj = []
        iou_per_obj = []

        n_pixels = (pred_masks_gpu[:, 0] > 0.0).detach().sum(dim=(-1, -2)).cpu()#.numpy().astype(np.uint8)
        iou_per_obj = torch.ones_like(n_pixels)
        current_out["n_pixels_pos"] = n_pixels
        current_out["iou"] = iou_per_obj
